...
description: >-
  ...
  tasks/, or review findings) to act on. Writes idiomatic, secure Laravel 9 +
  Blade/AdminLTE code that matches the existing conventions, reads the real code
...
1. **`laravel9`** — the build hub. Apply its always-active core rules (version
   ...
   `frontend.md`, `testing.md`, or `access-control.md`. Never load all of them.
...
3. **Make the smallest correct edit.** Preserve the existing request flow, Blade
   layout, and jQuery behavior. Match Bootstrap 4 / AdminLTE 3 markup. Don't
   rename routes/tables/columns without grepping every usage.
